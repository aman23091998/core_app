...
public class DrawerAdapter extends RecyclerView.Adapter<DrawerAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
       if(holder.viewType == ROW_TYPE){
         String rowtext = rows.get (position -1 );
          ...
          /*
          if you wish to have an image along with each row
    ...
    public static class ViewHolder extends  RecyclerView.ViewHolder {
     ...
     /*
        if including icons in each rom
        @Bind(R.id.drawer_row_icon) ImageView imageView; */
        @Bind(R.id.drawer_row_text) TextView textView;
